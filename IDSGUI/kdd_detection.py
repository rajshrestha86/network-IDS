...
class ids():
	...
	def predict(self, kdd_test_dataset):
		...
		for i in pred:
			...
			if i==None:
				break
			if kdd_test_dataset.values[:, [41]][count] == 'normal':
	...
	def main(self, traindir, testdir, teststd):
		...
		return 0
		# kdd_test_dataset.to_csv('./dataset/unlabeled_duration.txt',header=None, index=None)
		# kdd_dataset.to_csv('./dataset/unlabeled_duration(test).txt',header=None, index=None)
		# init_dataset_features.to_csv('./dataset/init_centroid.txt', header=None, index=None)
